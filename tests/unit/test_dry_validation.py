...
class TestDRYValidation:
    ...
    def test_consistent_import_patterns(self, package_root):
        ...
        internal_imports = [
            imp for imp in import_patterns["from"] if "dotmac_infra" in imp
        ]
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef) and node.name.endswith("SDK"):
                        ...
                        sdk_classes.append(
                            {"name": node.name, "bases": bases, "file": str(file_path)}
                        )
            except Exception:
                continue
...
class TestCodeQuality:
    ...
    def test_all_modules_have_docstrings(self):
        ...
        package_root = (
            Path(__file__).parent.parent.parent / "dotmac_infra" / "dotmac_infra"
        )
