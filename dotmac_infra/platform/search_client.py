...
class SearchClient:
    ...
    async def suggest(
        self, db: Session, partial_query: str, entity_type: str, limit: int = 10
    ) -> List[str]:
    ...
    def _build_filters_sql(
        self, filters: Dict[str, Any], entity: SearchableEntity
    ) -> str:
    ...
    async def get_search_stats(self, db: Session) -> Dict[str, Any]:
        ...
        for entity_type, entity in self.searchable_entities.items():
            ...
            result = await self._execute_sql(
                db, stats_query, {"tenant_id": self.tenant_id}
            )
            row = result.fetchone()
            ...
            stats[entity_type] = {
                ...
                "index_coverage": (row.indexed_rows / row.total_rows * 100)
                if row.total_rows > 0
                else 0,
            }
