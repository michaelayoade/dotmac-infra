...
class MockObservabilityClient:
    ...
    def error(
        self, message: str, error: Exception = None, metadata: Dict[str, Any] = None
    ):
...
class MockMonitoringClient:
    ...
    def create_histogram(
        self, name: str, description: str, buckets: list, labels: Dict[str, str] = None
    ):
        self.metrics[name] = {
            "type": "histogram",
            "buckets": buckets,
    ...
    def increment_counter(
        ...
        if name in self.metrics:
            ...
            print(
                f"[METRIC] Counter {name} incremented by {value} | Labels: {labels or {}}"
            )
    ...
    def observe_histogram(self, name: str, value: float, labels: Dict[str, str] = None):
        if name in self.metrics:
            print(
                f"[METRIC] Histogram {name} observed {value} | Labels: {labels or {}}"
            )
...
class MockAlertingClient:
    ...
    async def create_notification_config(self, config):
        print(
            f"[ALERT] Created notification config: {config['name']} | Channel: {config['channel']}"
        )
        return config
...
class ObservabilityManager:
    ...
    async def initialize(self):
        ...
        alerting_config = {
            ...
            "globalLabels": {"environment": environment, "service": "isp-framework"},
        }
        ...
        self.observability.info(
            "Observability Plane initialized successfully",
            {
                "tenant_id": tenant_id,
                "environment": environment,
    ...
    async def _setup_core_observability(self):
        ...
        # Customer metrics
        self.monitoring.create_counter(
            "isp_customers_total", "Total number of customers"
        )
        self.monitoring.create_counter(
            "isp_customer_registrations_total", "Total customer registrations"
        )
        self.monitoring.create_gauge(
            "isp_active_customers", "Number of active customers"
        )
        self.monitoring.create_gauge(
            "isp_revenue_mrr_cents", "Monthly recurring revenue in cents"
        )

        # Service metrics
        self.monitoring.create_counter(
            "isp_services_activated_total", "Total services activated"
        )
        self.monitoring.create_gauge("isp_active_services", "Number of active services")
        self.monitoring.create_counter(
            "isp_service_failures_total", "Total service failures"
        )
        ...
        self.monitoring.create_histogram(
            "isp_api_request_duration_ms",
        ...
        # Billing metrics
        self.monitoring.create_counter(
            "isp_billing_processed_total", "Total billing transactions processed"
        )
        self.monitoring.create_counter(
            "isp_billing_failures_total", "Total billing failures"
        )
        self.monitoring.create_gauge(
            "isp_billing_revenue_total_cents", "Total billing revenue in cents"
        )

        # Network metrics
        self.monitoring.create_gauge(
            "isp_network_latency_ms", "Network latency in milliseconds"
        )
        self.monitoring.create_gauge(
            "isp_network_packet_loss_percent", "Network packet loss percentage"
        )
        self.monitoring.create_gauge(
            "isp_network_bandwidth_mbps", "Network bandwidth in Mbps"
        )
        self.monitoring.create_gauge(
            "isp_network_uptime_percent", "Network uptime percentage"
        )
        ...
        self.observability.info(
            "Core observability setup completed",
            {
                "metrics_created": 15,
                "health_checks": 3,
                "alert_rules": 4,
    ...
    async def trace_operation(
        self, operation_name: str, metadata: Dict[str, Any] = None
    ):
    ...
    def track_api_request(
        ...
        if status_code >= 400:
            self.monitoring.increment_counter(
                "isp_api_errors_total",
                1,
                {"method": method, "path": path, "status_code": str(status_code)},
            )

        self.monitoring.observe_histogram(
            "isp_api_request_duration_ms", duration_ms, {"method": method, "path": path}
        )
    ...
    def get_health_status(self):
        ...
        return {
            "status": "healthy",
            "observability": self.observability.connected
            if self.observability
            else False,
