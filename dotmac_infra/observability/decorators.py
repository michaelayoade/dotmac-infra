        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if obs_manager.observability:
                    ...
                    result = await obs_manager.observability.trace(
                        op_name, traced_operation
                    )
                else:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                obs_manager.monitoring.observe_histogram(
                    f"isp_{metric_name}_duration_ms", duration_ms, metric_labels
                )

                obs_manager.monitoring.increment_counter(
                    f"isp_{metric_name}_total", 1, metric_labels
                )
...
def alert_on_failure(
    alert_name: str, severity: str = "warning", labels: Dict[str, str] = None
):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if metric_type == "counter":
                obs_manager.monitoring.increment_counter(
                    f"isp_business_{metric_name}_total", value, labels
                )
            elif metric_type == "gauge":
                obs_manager.monitoring.set_gauge(
                    f"isp_business_{metric_name}", value, labels
                )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                obs_manager.observability.info(
                    f"Customer operation completed: {operation_type}",
                    {
                        "operation": operation_type,
