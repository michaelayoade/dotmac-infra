...
class OperationsPlaneIntegrationTests:
    ...
    def __init__(self, tenant_id: str = "test_tenant"):
        ...
        self.test_context = SecurityContext(
            ...
            permissions=[
                ...
                Permission.STATE_MACHINE_CREATE,
                Permission.STATE_MACHINE_EXECUTE,
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Operations Plane integration tests completed: {success_rate:.1f}% success rate"
        )
        return summary
    ...
    async def test_workflow_engine_basic(self):
        ...
        workflow_data = await self.workflow_sdk.get_workflow(
            workflow_id, self.test_context
        )
    ...
    async def test_task_scheduler_basic(self):
        ...
        recurring_task_id = await self.task_sdk.schedule_recurring_task(
            ...
            schedule_config={"interval_seconds": 60, "max_executions": 5},
            context=self.test_context,
        )
    ...
    async def test_state_machine_basic(self):
        ...
        # Define states
        states_config = [
            {"state_id": "initial", "name": "Initial State", "state_type": "initial"},
            {
                "state_id": "processing",
                "name": "Processing State",
                "state_type": "intermediate",
            },
            {"state_id": "completed", "name": "Completed State", "state_type": "final"},
        ]
    ...
    async def test_automation_engine_basic(self):
        ...
        trigger_config = {"type": "event", "event_pattern": "test.event.*"}
        ...
        execution_id = await self.automation_sdk.execute_automation_rule(
            rule_id=rule_id,
            trigger_data={
                "event_data": {"priority": 8, "message": "Test high priority event"}
            },
            context=self.test_context,
        )
    ...
    async def test_cross_sdk_composition(self):
        ...
        for sdk in [
            ...
            self.task_sdk,
            self.job_sdk,
            self.state_machine_sdk,
            self.automation_sdk,
        ]:
            assert hasattr(sdk, "db_sdk"), f"{sdk.__class__.__name__} missing db_sdk"
            assert hasattr(sdk, "cache_sdk"), (
                f"{sdk.__class__.__name__} missing cache_sdk"
            )
            assert hasattr(sdk, "event_sdk"), (
                f"{sdk.__class__.__name__} missing event_sdk"
            )
            assert hasattr(sdk, "observability_sdk"), (
                f"{sdk.__class__.__name__} missing observability_sdk"
            )
    ...
    async def test_workflow_with_tasks(self):
        ...
        await self.workflow_sdk.register_step_handler(
    ...
    async def test_automation_with_workflows(self):
        ...
        actions_config = [
            {
                "type": "workflow",
                "workflow_id": workflow_id,
                "input_data": {"triggered_by": "automation"},
            }
        ]
        ...
        execution_id = await self.automation_sdk.execute_automation_rule(
            rule_id=rule_id, trigger_data={"source": "test"}, context=self.test_context
        )
    ...
    async def test_state_machine_with_events(self):
        ...
        # Verify final state
        status = await self.state_machine_sdk.get_instance_status(
            instance_id, self.test_context
        )
    ...
    async def test_sdk_health_checks(self):
        ...
        for sdk_name, sdk in sdks:
            ...
            assert health_status["status"] == "healthy", (
                f"{sdk_name} health check failed"
            )
