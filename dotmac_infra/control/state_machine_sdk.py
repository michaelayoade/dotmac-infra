...
class State:
    def __init__(
        ...
        entry_actions: List[Dict] = None,
        exit_actions: List[Dict] = None,
    ):
...
class StateMachineSDK(BaseOperationsSDK):
    ...
    async def create_state_machine(
        ...
        name: str,
        description: str,
        states_config: List[Dict],
        transitions_config: List[Dict],
        initial_state: str,
        ...
        try:
            ...
            await self.cache_sdk.set(
                f"state_machine:{self.tenant_id}:{machine_id}", machine_data, ttl=3600
            )
            await self.event_sdk.publish(
                "state_machine.created", {"machine_id": machine_id, "name": name}
            )
    ...
    async def start_instance(
        self, machine_id: str, context_data: Dict, context: SecurityContext
    ) -> str:
        ...
        try:
            ...
            machine_data = await self.cache_sdk.get(
                f"state_machine:{self.tenant_id}:{machine_id}"
            )
            if not machine_data:
            ...
            # Publish event
            await self.event_sdk.publish(
                "state_machine.instance.started",
                {
                    "instance_id": instance_id,
                    "machine_id": machine_id,
    ...
    async def send_event(
        self, instance_id: str, event: str, event_data: Dict, context: SecurityContext
    ) -> bool:
        ...
        try:
            ...
            if target_state.get("state_type") == StateType.FINAL.value:
                ...
                await self.event_sdk.publish(
                    "state_machine.instance.completed",
    ...
    async def get_instance_status(
        self, instance_id: str, context: SecurityContext
    ) -> Optional[Dict]:
