...
class WorkflowEngineSDK(BaseOperationsSDK):
    ...
    async def create_workflow(
        ...
        try:
            ...
            if not await self.validate_operation_permissions(
                "workflow_engine.create", [Permission.WORKFLOW_CREATE], context
            ):
            ...
            for i, step_config in enumerate(steps_config):
                step = WorkflowStep(
                    ...
                    execution_mode=ExecutionMode(
                        step_config.get("execution_mode", "sequential")
                    ),
            ...
            await self.db_sdk.execute(
                ...
                (
                    ...
                    json.dumps(workflow.to_dict()["steps"]),
                    workflow.priority.value,
                    workflow.timeout_seconds,
                    workflow.compensation_enabled,
    ...
    async def execute_workflow(
        self, workflow_id: str, input_data: Dict[str, Any], context: SecurityContext
    ) -> str:
        """
        ...
        try:
            # Validate permissions
            if not await self.validate_operation_permissions(
                "workflow_engine.execute", [Permission.WORKFLOW_EXECUTE], context
            ):
            ...
            if len(self.running_workflows) < self.max_concurrent_workflows:
                execution_task = asyncio.create_task(
                    self._execute_workflow_instance(
                        workflow_data, execution_id, input_data, context
                    )
                )
        ...
        try:
            ...
            if workflow_data.get("parallel_execution", False):
                await self._execute_steps_parallel(
                    workflow_data, execution_context, context
                )
            else:
    ...
    async def _execute_steps_sequential(
        ...
        for step_id in step_order:
            ...
            step_result = await self._execute_step(
                step_data, execution_context, context
            )
        ...
        try:
            ...
            if action_type == "job_queue":
                ...
                job_id = await self.job_queue_sdk.enqueue_job(
                    job_type=job_type, payload=payload, context=context
                )
    ...
    async def _check_step_dependencies(
        self, step_data: Dict[str, Any], execution_context: Dict[str, Any]
    ) -> bool:
    ...
    async def _compensate_workflow(
        ...
        logger.info(
            f"Starting compensation for workflow execution {execution_context['execution_id']}"
        )
    ...
    async def get_workflow(
        self, workflow_id: str, context: SecurityContext
        ...
        try:
            ...
            if workflow_record:
                ...
                workflow_data["metadata"] = (
                    json.loads(workflow_data["metadata"])
                    if workflow_data["metadata"]
                    else {}
                )
    ...
    async def register_step_handler(self, action_type: str, handler_func: Callable):
