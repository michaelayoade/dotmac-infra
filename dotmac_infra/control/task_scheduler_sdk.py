...
class TaskSchedulerSDK(BaseOperationsSDK):
        ...
        **kwargs,
        ...
        try:
            ...
            if not await self.validate_operation_permissions(
                f"task_scheduler.{action_type}", [Permission.TASK_CREATE], context
            ):
                raise PermissionError(
                    f"Insufficient permissions to schedule {action_type} task"
                )
            ...
            await self.db_sdk.execute(
                ...
                (
                    ...
                    self._calculate_next_cron_run(cron_expression),
                ),
            )
    ...
    async def schedule_recurring_task(
        ...
        try:
            ...
            logger.info(
                f"Recurring task {task_id} ({name}) scheduled every {interval_seconds}s"
            )
            return task_id
    ...
    async def _complete_task_execution(
        self, task_data: Dict[str, Any], success: bool, error: Optional[str] = None
    ):
