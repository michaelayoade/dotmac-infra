...
class AutomationRule:
    ...
    def __init__(self, rule_id: str, name: str, description: str,
                 ...
                 actions_config: List[Dict], priority: OperationPriority = OperationPriority.NORMAL):
...
class AutomationEngineSDK(BaseOperationsSDK):
    ...
    async def create_automation_rule(self, name: str, description: str,
                                   trigger_config: Dict, condition_config: Dict,
                                   actions_config: List[Dict], context: SecurityContext,
        ...
        try:
            ...
            await self.db_sdk.execute(
                ...
                (rule_id, self.tenant_id, name, description,
                 json.dumps(trigger_config), json.dumps(condition_config),
                 json.dumps(actions_config), priority.value, rule.status.value, rule.created_at)
            )
            
            # Cache and activate rule
            self.active_rules[rule_id] = rule
            await self.cache_sdk.set(f"automation_rule:{self.tenant_id}:{rule_id}", 
                                   rule.__dict__, ttl=3600)
            
            # Set up trigger if needed
            await self._setup_trigger(rule)
            
            # Publish event
            await self.event_sdk.publish("automation.rule.created", {
                "rule_id": rule_id, "name": name, "trigger_type": trigger_config.get("type")
            })
            
            await self.observability_sdk.log_info(f"Automation rule {rule_id} ({name}) created")
        ...
        if trigger_type == "schedule":
            ...
            if event_pattern:
                await self.event_sdk.subscribe(
                    event_pattern,
                    lambda event_data, rule_id=rule.rule_id: self._handle_event_trigger(rule_id, event_data)
                )
    
    async def execute_automation_rule(self, rule_id: str, trigger_data: Dict,
        ...
        try:
            ...
            await self.db_sdk.execute(
                """INSERT INTO operations_automation_executions 
                   ...
                   VALUES (%s, %s, %s, %s, %s, %s)""",
                (execution_id, rule_id, self.tenant_id, execution["status"],
                 json.dumps(trigger_data), execution["started_at"])
            )
    ...
    async def _execute_rule_async(self, rule: AutomationRule, execution: Dict, context: SecurityContext):
        """Execute automation rule asynchronously"""
        try:
            execution["status"] = ExecutionStatus.RUNNING.value
            
            # Evaluate conditions
            if not await self._evaluate_conditions(rule.condition_config, execution["trigger_data"]):
            ...
            for i, action_config in enumerate(rule.actions_config):
                try:
                    result = await self._execute_action(action_config, execution["trigger_data"], context)
                    action_results.append({"action_index": i, "result": result, "status": "completed"})
                    execution["actions_completed"] += 1
                    
                except Exception as e:
                    action_results.append({"action_index": i, "error": str(e), "status": "failed"})
                    logger.error(f"Action {i} failed in execution {execution['execution_id']}: {e}")
            ...
            await self.event_sdk.publish("automation.execution.completed", {
            ...
            await self.event_sdk.publish("automation.execution.failed", {
                "execution_id": execution["execution_id"],
                "rule_id": rule.rule_id,
                "error": str(e)
            })
    ...
    async def _evaluate_conditions(self, condition_config: Dict, trigger_data: Dict) -> bool:
        ...
        if condition_type == "always":
            ...
            if evaluator_name in self.condition_evaluators:
                return await self.condition_evaluators[evaluator_name](condition_config, trigger_data)
        
        return False
    
    async def _execute_action(self, action_config: Dict, trigger_data: Dict, context: SecurityContext) -> Any:
    ...
    async def _execute_workflow_action(self, action_config: Dict, trigger_data: Dict, context: SecurityContext):
    ...
    async def _execute_task_action(self, action_config: Dict, trigger_data: Dict, context: SecurityContext):
        """Execute task action"""
    ...
    async def _execute_notification_action(self, action_config: Dict, trigger_data: Dict, context: SecurityContext):
    ...
    async def _execute_api_call_action(self, action_config: Dict, trigger_data: Dict, context: SecurityContext):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request(method, url, json=merged_payload, headers=headers) as response:
    ...
    async def _get_automation_rule(self, rule_id: str) -> Optional[AutomationRule]:
        ...
        # Check cache
        cached_rule = await self.cache_sdk.get(f"automation_rule:{self.tenant_id}:{rule_id}")
        if cached_rule:
    ...
    async def _complete_execution(self, execution: Dict):
        ...
        await self.db_sdk.execute(
            ...
            (execution["status"], execution.get("completed_at"),
             json.dumps(execution.get("result")), execution.get("error"),
             execution["execution_id"])
        )
    
    async def get_execution_status(self, execution_id: str, context: SecurityContext) -> Optional[Dict]:
