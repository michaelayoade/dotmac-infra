...
class JobQueueSDK(BaseOperationsSDK):
    ...
    async def enqueue_job(
        ...
        try:
            ...
            if not await self.validate_operation_permissions(
                f"job_queue.{job_type.value}", [Permission.JOB_CREATE], context
            ):
                raise PermissionError(
                    f"Insufficient permissions to enqueue {job_type.value} job"
                )
    ...
    async def get_job(
        self, job_id: str, context: SecurityContext
        ...
        try:
            ...
            if job_record:
                ...
                # Parse JSON fields
                job_data["payload"] = (
                    json.loads(job_data["payload"]) if job_data["payload"] else {}
                )
                job_data["metadata"] = (
                    json.loads(job_data["metadata"]) if job_data["metadata"] else {}
                )
                job_data["result"] = (
                    json.loads(job_data["result"]) if job_data["result"] else None
                )
    ...
    async def register_job_handler(self, job_type: JobType, handler_func: Callable):
    ...
    async def _handle_job_failure(self, job_data: Dict[str, Any], error_message: str):
        ...
        if retry_count < max_retries:
            ...
            logger.error(
                f"Job {job_id} moved to dead letter queue after {retry_count} retries"
            )
