...
class AddressSDK(BaseSDK):
    ...
    async def create_address(
        ...
        normalized_data = {
            ...
            "address_type": AddressType(
                address_data.get("address_type", AddressType.BILLING.value)
            ),
            "status": AddressStatus(
                address_data.get("status", AddressStatus.ACTIVE.value)
            ),
    ...
    @cache_result(
        ttl=300, key_func=lambda self, ctx, address_id, db: f"address:{address_id}"
    )
    @trace_operation("address.get")
    async def get_address(
        self, operation_context: OperationContext, address_id: str, db: Session
    ...
    async def update_address(
        ...
        updated_address = await self.database.update(
            "addresses", address_id, normalized_updates, db
        )
    ...
    async def list_addresses(
        ...
        if filters:
            ...
            if "address_type" in filters:
                query_filters["address_type"] = AddressType(
        ...
        addresses = await self.database.list(
            "addresses", query_filters, limit, offset, db
        )
    ...
    async def verify_address(
        ...
        if "corrected_address" in verification_data:
            ...
            verification_updates.update(
                {
                    "street_address": corrected.get(
                        "street_address", existing_address["street_address"]
                    ),
                    "city": corrected.get("city", existing_address["city"]),
                    "state": corrected.get("state", existing_address["state"]),
                    "postal_code": corrected.get(
        ...
        # Update address
        updated_address = await self.database.update(
            "addresses", address_id, verification_updates, db
        )
    ...
    async def get_addresses_for_entity(
        ...
        entity_addresses = await self.database.list(
            f"{entity_type}_addresses",
        ...
        for entity_address in entity_addresses:
            address = await self.get_address(
                operation_context, entity_address["address_id"], db
            )
            if address:
