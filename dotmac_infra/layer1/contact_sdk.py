...
class ContactSDK(BaseSDK):
    ...
    async def create_contact(
        ...
        normalized_data = {
            ...
            "contact_type": ContactType(
                contact_data.get("contact_type", ContactType.INDIVIDUAL.value)
            ),
            "role": ContactRole(contact_data.get("role", ContactRole.PRIMARY.value)),
            "status": ContactStatus(
                contact_data.get("status", ContactStatus.ACTIVE.value)
            ),
    ...
    @cache_result(
        ttl=300, key_func=lambda self, ctx, contact_id, db: f"contact:{contact_id}"
    )
    @trace_operation("contact.get")
    async def get_contact(
        self, operation_context: OperationContext, contact_id: str, db: Session
    ...
    async def update_contact(
        ...
        updated_contact = await self.database.update(
            "contacts", contact_id, normalized_updates, db
        )
    ...
    async def list_contacts(
        ...
        if filters:
            ...
            if "contact_type" in filters:
                query_filters["contact_type"] = ContactType(
        ...
        contacts = await self.database.list(
            "contacts", query_filters, limit, offset, db
        )
