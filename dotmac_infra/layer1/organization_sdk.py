...
class OrganizationSDK(BaseSDK):
    ...
    @require_permission(
        Permission.ORGANIZATION_CREATE.value, ResourceType.ORGANIZATION.value
    )
    ...
    async def create_organization(
        ...
        if organization_data.get("tax_id"):
            tax_validation = self._validate_tax_id(
                organization_data["tax_id"], organization_data.get("country", "US")
            )
        ...
        normalized_data = {
            ...
            "status": OrganizationStatus(
                organization_data.get("status", OrganizationStatus.ACTIVE.value)
            ),
            "name": organization_data["name"],
            "legal_name": organization_data.get(
                "legal_name", organization_data["name"]
            ),
            "dba_name": organization_data.get("dba_name"),
            "registration_number": organization_data.get("registration_number"),
            "tax_id": tax_validation["formatted"]
            if tax_validation
            else organization_data.get("tax_id"),
            ...
            "created_by": operation_context.user_context.user_id
    ...
    @require_permission(
        Permission.ORGANIZATION_READ.value, ResourceType.ORGANIZATION.value
    )
    @cache_result(
        ttl=300, key_func=lambda self, ctx, org_id, db: f"organization:{org_id}"
    )
    @trace_operation("organization.get")
    async def get_organization(
        self, operation_context: OperationContext, organization_id: str, db: Session
    ) -> Optional[Dict[str, Any]]:
        """
        Get organization by ID with caching and security
        """
        organization = await self.database.get_by_id(
            "organizations", organization_id, db
        )
    ...
    @require_permission(
        Permission.ORGANIZATION_UPDATE.value, ResourceType.ORGANIZATION.value
    )
    ...
    async def update_organization(
        ...
        existing_organization = await self.get_organization(
            operation_context, organization_id, db
        )
        ...
        normalized_updates["updated_by"] = (
            operation_context.user_context.user_id
            if operation_context.user_context
            else None
        )

        # Update using Platform Database SDK
        updated_organization = await self.database.update(
            "organizations", organization_id, normalized_updates, db
        )
        ...
        await self._index_for_search(
            ...
            organization_id,
            {
                ...
                "registration_number": updated_organization.get(
                    "registration_number", ""
                ),
    ...
    @require_permission(
        Permission.ORGANIZATION_DELETE.value, ResourceType.ORGANIZATION.value
    )
    ...
    async def delete_organization(
        ...
        child_organizations = await self.database.list(
            "organizations",
            {
        ...
        await self.database.update(
            ...
            organization_id,
            {
                ...
                "deleted_by": operation_context.user_context.user_id
                if operation_context.user_context
                else None,
            },
            db,
    ...
    async def list_organizations(
        ...
        if filters:
            ...
            if "organization_type" in filters:
                query_filters["organization_type"] = OrganizationType(
            ...
            if "parent_organization_id" in filters:
                query_filters["parent_organization_id"] = filters[
        ...
        organizations = await self.database.list(
            "organizations", query_filters, limit, offset, db
        )
    ...
    async def get_organization_hierarchy(
        ...
        hierarchy = {"organization": organization, "parent": None, "children": []}

        # Get parent organization
        if include_parent and organization.get("parent_organization_id"):
            parent = await self.get_organization(
                operation_context, organization["parent_organization_id"], db
            )
            hierarchy["parent"] = parent

        # Get child organizations
        if include_children:
            children = await self.database.list(
                "organizations",
                {
                    "parent_organization_id": organization_id,
                    "status": OrganizationStatus.ACTIVE.value,
    ...
    async def verify_organization(
        ...
        verification_updates = {
            ...
            "updated_at": datetime.utcnow(),
            "updated_by": operation_context.user_context.user_id
            if operation_context.user_context
            else None,
        }
        ...
        if "verified_data" in verification_data:
            ...
            verification_updates.update(
                {
                    "legal_name": verified.get(
                        "legal_name", existing_organization["legal_name"]
                    ),
                    "registration_number": verified.get(
                        "registration_number",
                        existing_organization["registration_number"],
                    ),
                    "tax_id": verified.get("tax_id", existing_organization["tax_id"]),
                    "industry": verified.get(
                        "industry", existing_organization["industry"]
                    ),
                    "industry_code": verified.get(
                        "industry_code", existing_organization["industry_code"]
                    ),
                }
            )
        ...
        # Update organization
        updated_organization = await self.database.update(
            "organizations", organization_id, verification_updates, db
        )
    ...
    async def get_organization_contacts(
        ...
        organization_contacts = await self.database.list(
            "organization_contacts", query_filters, limit=100, offset=0, db=db
        )
    ...
    async def add_organization_contact(
        ...
        relationship = await self.database.create(
