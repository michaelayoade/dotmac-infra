...
class PhoneSDK(BaseSDK):
    ...
    async def create_phone(
        ...
        if not validation["is_valid"]:
            raise ValueError(
                f"Invalid phone number format: {phone_data['phone_number']}"
            )
        ...
        normalized_data = {
            ...
            "phone_type": PhoneType(
                phone_data.get("phone_type", PhoneType.MOBILE.value)
            ),
    ...
    async def get_phone(
        self, operation_context: OperationContext, phone_id: str, db: Session
    ...
    async def update_phone(
        ...
        if "phone_number" in update_data:
            ...
            if not validation["is_valid"]:
                raise ValueError(
                    f"Invalid phone number format: {update_data['phone_number']}"
                )
        ...
        updated_phone = await self.database.update(
            "phones", phone_id, normalized_updates, db
        )
    ...
    async def verify_phone(
        ...
        # Update phone
        updated_phone = await self.database.update(
            "phones", phone_id, verification_updates, db
        )
    ...
    async def get_phones_for_entity(
        ...
        entity_phones = await self.database.list(
            f"{entity_type}_phones",
        ...
        for entity_phone in entity_phones:
            phone = await self.get_phone(
                operation_context, entity_phone["phone_id"], db
            )
            if phone:
