...
class EmailSDK(BaseSDK):
    ...
    async def create_email(
        ...
        if not validation["is_valid"]:
            raise ValueError(
                f"Invalid email address format: {email_data['email_address']}"
            )
        ...
        normalized_data = {
            ...
            "email_type": EmailType(
                email_data.get("email_type", EmailType.PERSONAL.value)
            ),
            ...
            "can_receive_notifications": email_data.get(
    ...
    async def get_email(
        self, operation_context: OperationContext, email_id: str, db: Session
    ...
    @cache_result(
        ttl=300,
        key_func=lambda self, ctx, email_address, db: f"email:address:{email_address}",
    )
    @trace_operation("email.get_by_address")
    async def get_email_by_address(
        self, operation_context: OperationContext, email_address: str, db: Session
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        email = await self.database.get_by_field(
            "emails", "email_address", normalized_email, db
        )
    ...
    async def update_email(
        ...
        if "email_address" in update_data:
            ...
            if not validation["is_valid"]:
                raise ValueError(
                    f"Invalid email address format: {update_data['email_address']}"
                )
        ...
        updated_email = await self.database.update(
            "emails", email_id, normalized_updates, db
        )
    ...
    async def list_emails(
        ...
        if filters:
            ...
            if "can_receive_marketing" in filters:
                query_filters["can_receive_marketing"] = filters[
                    "can_receive_marketing"
                ]
            if "can_receive_notifications" in filters:
                query_filters["can_receive_notifications"] = filters[
    ...
    async def verify_email(
        ...
        # Update email
        updated_email = await self.database.update(
            "emails", email_id, verification_updates, db
        )
    ...
    async def get_emails_for_entity(
        ...
        entity_emails = await self.database.list(
            f"{entity_type}_emails",
        ...
        for entity_email in entity_emails:
            email = await self.get_email(
                operation_context, entity_email["email_id"], db
            )
            if email:
    ...
    async def send_verification_email(
        ...
        verification_url = (
            f"{verification_url_base}?token={verification_token}&email_id={email_id}"
        )
    ...
    async def record_email_bounce(
        ...
        # Update email
        updated_email = await self.database.update(
            "emails", email_id, bounce_updates, db
        )
